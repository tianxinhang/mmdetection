...
class AnchorHead(BaseDenseHead):
    ...
    def _get_targets_single_2(self,
                            flat_anchors,
                            valid_flags,
                            gt_bboxes,
                            gt_bboxes_ignore,
        ...
        bbox_targets = torch.zeros_like(anchors)
        bbox_weights = torch.zeros_like(anchors)
        labels = anchors.new_full((num_valid_anchors, ),            # bbox照常训练
                                  self.background_label,
                                  dtype=torch.long)
        label_weights = anchors.new_zeros(num_valid_anchors, dtype=torch.float)
    ...
    def loss_single_2(self, cls_score, bbox_pred, anchors, labels, label_weights,          #labels, label_weights, bbox_targets这些长度还是anchor size，但只有256个不是0
        ...
        Args:
            cls_score (Tensor): Box scores for each scale level                 single scale level. N是图片数
                Has shape (N, num_anchors * num_classes, H, W).
            bbox_pred (Tensor): Box energies / deltas for each scale
                level with shape (N, num_anchors * 4, H, W).
            anchors (Tensor): Box reference for each scale level with shape
...
#                     # bbox_weights[i,j,:] = 0.0             # 这里需不需要呢？？？？？？？？
        ...
        label_weights = label_weights.reshape(-1)
        cls_score = cls_score.permute(0, 2, 3,
                                      1).reshape(-1, self.cls_out_channels) #self.cls_out_channels = 1,这一步为了把一个点的三个anchor聚集起来。

        loss_cls = self.loss_cls(
            cls_score, labels, label_weights, avg_factor=num_total_samples)             #weight就相当于记录，哪些需要进入loss计算的
        # regression loss
        bbox_targets = bbox_targets.reshape(-1, 4)
        bbox_weights = bbox_weights.reshape(-1, 4)
        bbox_pred = bbox_pred.permute(0, 2, 3, 1).reshape(-1, 4)
    ...
    def loss_2(self,
        ...
        anchor_list, valid_flag_list = self.get_anchors(
            featmap_sizes, img_metas, device=device)
        label_channels = self.cls_out_channels if self.use_sigmoid_cls else 1
        cls_reg_targets = self.get_targets_2(
            anchor_list,
            valid_flag_list,
            gt_bboxes,
            img_metas,
...
#         semantic_preds = [semantic_pred]*len(cls_scores)
    ...
    def get_bboxes_5(self,
        ...
        for img_id in range(len(img_metas)):
            ...
            # semantic_pred_single = semantic_pred[img_id]
            gt_semantic_seg_single = gt_semantic_seg[img_id]         #v5.0
            img_shape = img_metas[img_id]['img_shape']
            scale_factor = img_metas[img_id]['scale_factor']
            proposals = self._get_bboxes_single_5(cls_score_list, bbox_pred_list,
                                                mlvl_anchors, img_shape,
                                                scale_factor,gt_semantic_seg_single,cfg,rescale)    #v5.0
    ...
    def get_bboxes_2(self,
        ...
        for img_id in range(len(img_metas)):
            ...
            img_shape = img_metas[img_id]['img_shape']
            scale_factor = img_metas[img_id]['scale_factor']
            proposals = self._get_bboxes_single_2(cls_score_list, bbox_pred_list,
                                                mlvl_anchors, img_shape,
                                                scale_factor,semantic_num_classes,semantic_pred_single,cfg,rescale)
            result_list.append(proposals)
        return result_list
